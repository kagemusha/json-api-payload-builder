...
(function(root, factory) {
  ...
  var buildPayload = function(root, json, options) {
    ...
    var rootArray = isArray(clonedJson) ? clonedJson : [ clonedJson ];
    var payload = {};
  ...
  function link(parentType, key, parentObj, childObj) {
    ...
    var childType = childBaseType;

    //implicitly assuming a type will not have both mappings and polymorphic types
    if (isPolymorphic(childBaseType)) {
      childType = childObj.type;
    } else if(hasMapping(childBaseType)) {
      childType = typeMapping(childBaseType);
    }
  ...
  function hasMapping(childBaseType){
    return !!(_mappings[childBaseType]);
  }

  function typeMapping(childBaseType){
    return _mappings[childBaseType];
  }
